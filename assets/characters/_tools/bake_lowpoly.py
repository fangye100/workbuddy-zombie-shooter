...
🔴🔴 DEPRECATED（2026-09-18 用户拍板弃用）——请改用 `rebake_e01_lod1.py`（空间映射转移烘焙）。
...
替代方案（现行）：
  - rebake_e01_lod1.py：空间映射转移烘焙（低模 UV 像素 → 3D 点 → 高模最近位置 →
    采原生 4096² 贴图色），环境侧同款方法见 assets/environment/_tools/env_transfer.py。
  - 规划中（路线 A）：decimate 保 UV 减面——减面时继承高模 wedge UV，
    LOD 直接采样原生贴图，上色工序整个消失，全 LOD 复用同一张贴图。

以下原实现仅作历史参考保留，勿在新流程中调用。
--------------------------------------------------------------------------------
