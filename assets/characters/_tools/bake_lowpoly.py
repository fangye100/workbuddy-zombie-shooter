...
🔴🔴 DEPRECATED（2026-09-18 用户拍板弃用）——角色侧请改用现行工具，别再走本脚本。

替代方案（都已在库，按用途选）：
  - `decimate_uvkeep.py`（**路线 A 现行**）：焊点 + 保纹理 QEM + wedge 顶点分裂 +
    内嵌原生 4096² 贴图，一条龙产出 LOD1。低模 UV 与原生意一致 ⇒ 无需任何烘焙。
  - `bake_texture_transfer.py`：空间映射转移烘焙（低模 UV 像素 → 3D 点 → 高模最近位置 →
    采原生贴图色）。在低模 UV 与原生无继承关系时才需要，现已被路线 A 取代。
  - 环境侧：`assets/environment/_tools/env_transfer.py`（同款转移思路），
    其低模输入是 `assets/environment/_tools/env_bake.py` 产出的 xatlas UV OBJ。

弃用原因（两大缺陷）：
  1. 色源用顶点色而非 Texture：顶点色本身是「原贴图烧到顶点」的有损压缩
     （80k 面烧到 900 面，每顶点一色），细节全灭——1024² 贴图 uniq 268 vs 原生 570，
     产出的 LOD1/2/3 就是用户看到的「糊」。
  2. 每次用 xatlas 现展 UV：布局与任何既有贴图无继承关系，贴图无法复用、
     无法「只换贴图不换模型」，每只角色每轮都要重新烘焙，浪费。
     （注：**UV 展开这一步本身**仍被环境链路复用 —— `env_bake.py` 借它产出
     `tex/<ID>_tex.obj` 作为 `env_transfer.py` 的输入。那里要的是 OBJ，不是贴图。）

以下原实现仅作历史参考保留，勿在角色新流程中调用。
--------------------------------------------------------------------------------
