...
import { BUILTIN_MODELS, CHARACTER_HEIGHT_M, normalizeMeshHeight } from '../src/models';
...
describe('内置模型清单', () => {
  ...
  it('身高常量来自 roster.json 的 E-04 盾卫（2.05 m），不得就地魔改', () => {
    expect(CHARACTER_HEIGHT_M).toBeCloseTo(2.05, 6);
  });
  ...
  it('归一化函数由引擎层提供，编辑器不再维护第二份实现', () => {
    ...
    v[1] = 0.3;
    v[16] = 2.7;
    const out = normalizeMeshHeight({ vertices: v, indices: new Uint32Array([0, 1, 0]) }, CHARACTER_HEIGHT_M);
    expect(meshHeight(out)).toBeCloseTo(CHARACTER_HEIGHT_M, 5);
