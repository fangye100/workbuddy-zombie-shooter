...
const DICT: Record<string, string> = {
  ...
  // ---- 词典恢复（PR #15 bot 评审：此前编辑事故丢失的键，全量补回） ----
  ...
  '地面接触：有可信轨迹，生成后按检测结果启用': 'Ground contact: trusted trajectory found; enabled per detection after generation',
  '地面接触：能力受限 —— 源缺足底标定，本次为自由运动预览（不承诺世界锁脚）；SourceCalibration.markers 提供足底标记后可解锁': 'Ground contact: capability limited - source lacks foot calibration; this run is free-motion preview (no world foot-lock promised); unlockable once SourceCalibration.markers provides foot markers',
  ...
  '球体（skin）': 'Sphere (skin)',
  '生成失败：上一份结果已保留（见诊断）': 'Generation failed: the previous result is kept (see diagnostics)',
  ...
  '绑定': 'Binding',
  '结果待更新：重新「生成预览」后再应用 / 导出': 'Result stale: re-run Generate Preview before applying / exporting',
  ...
  '角色标定': 'Character Calibration',
  '角色（中心胶囊）': 'Character (center capsule)',
  '解析失败': 'Parse failed',
  '解析模型中…': 'Parsing model…',
  '诊断': 'Diagnostics',
  '该入口没有 sidecar 路径（场景物体不支持保存）': 'This entry has no sidecar path (saving is not supported for scene objects)',
  ...
  '输入就绪，点「生成预览」': 'Inputs ready - click Generate Preview',
  '边缘': 'Edges',
  '进入绑定': 'Enter Binding',
