...
export const MATERIAL_OPTIONS = [
  { label: t('0 · 僵尸绿（skin）'), value: 0 },
  { label: t('1 · 地面灰（cloth）'), value: 1 },
  { label: t('2 · 皮肤（skin）'), value: 2 },
  { label: t('3 · 金属（metal）'), value: 3 },
  { label: t('4 · 自发光（emissive）'), value: 4 },
  { label: t('5 · 布料（cloth）'), value: 5 },
];

export const TONEMAP_OPTIONS = [
  { label: t('None（线性截断）'), value: 0 },
  { label: t('Reinhard'), value: 1 },
  { label: t('ACES（Narkowicz 近似）'), value: 2 },
  { label: t('AgX（推荐）'), value: 3 },
];

export const DEBUG_OPTIONS = [
  { label: t('最终画面'), value: 0 },
  { label: t('Albedo'), value: 1 },
  { label: t('世界法线'), value: 2 },
  { label: t('NdotL（灰度）'), value: 3 },
  { label: t('分阶 ID'), value: 4 },
  { label: t('灯光贡献分解'), value: 5 },
  { label: t('描边 mask'), value: 6 },
  { label: t('UV 坐标（RG）'), value: 7 },
  { label: t('UV 棋盘格'), value: 8 },
];

export const EDIT_TARGETS = [
  { label: t('角色（中心胶囊）'), value: 0 },
  { label: t('地面'), value: 1 },
  { label: t('球体（skin）'), value: 2 },
  { label: t('立方体（metal）'), value: 3 },
  { label: t('圆柱（emissive）'), value: 4 },
  { label: t('胶囊（cloth）'), value: 5 },
];
...
export const PARAM_GROUPS: GroupDef[] = [
  ...
  },
  {
    ...
    tab: 'scene',
    controls: [
      { kind: 'slider', key: 'keyAzimuth', label: t('方位角'), min: -180, max: 180, step: 1 },
      {
        kind: 'slider',
        key: 'keyElevation',
        label: t('仰角'),
      ...
      { kind: 'slider', key: 'keyIntensity', label: t('强度'), min: 0, max: 3, step: 0.01 },
      { kind: 'color', key: 'keyColor', label: t('颜色') },
    ],
  },
  {
    id: 'fill',
    title: t('Fill 半球环境（不分阶）'),
    open: true,
    side: 'right',
    tab: 'scene',
    controls: [
      { kind: 'color', key: 'fillSkyColor', label: t('天空色') },
      { kind: 'slider', key: 'fillSkyIntensity', label: t('天空强度'), min: 0, max: 1.5, step: 0.01 },
      { kind: 'color', key: 'fillGroundColor', label: t('地面反弹色') },
      { kind: 'slider', key: 'fillGroundIntensity', label: t('地面强度'), min: 0, max: 1.5, step: 0.01 },
      { kind: 'color', key: 'ambientColor', label: t('常数环境') },
      { kind: 'slider', key: 'ambientIntensity', label: t('环境强度'), min: 0, max: 1, step: 0.01 },
    ],
  },
  {
    id: 'rim',
    ...
    tab: 'scene',
    controls: [
      { kind: 'color', key: 'rimColor', label: t('颜色') },
      {
        kind: 'slider',
        key: 'rimIntensity',
        label: t('强度'),
      ...
      { kind: 'slider', key: 'rimPower', label: t('锐度（power）'), min: 0.5, max: 8, step: 0.1 },
      {
        kind: 'slider',
        key: 'rimTopBias',
        label: t('顶边缘偏置'),
  ...
  {
    id: 'point',
    title: t('局部点光（调试用；位置取场景 Light 节点的世界坐标）'),
    open: false,
    side: 'right',
    tab: 'scene',
    controls: [
      { kind: 'toggle', key: 'pointEnabled', label: t('启用点光') },
      { kind: 'color', key: 'pointColor', label: t('颜色') },
      { kind: 'slider', key: 'pointIntensity', label: t('强度'), min: 0, max: 6, step: 0.01 },
      { kind: 'slider', key: 'pointRange', label: t('半径'), min: 0.5, max: 12, step: 0.1 },
      {
        kind: 'slider',
        key: 'fogDensity',
        label: t('雾密度'),
      ...
      { kind: 'color', key: 'fogColor', label: t('雾颜色') },
    ],
  },
  {
    id: 'toon',
    ...
    tab: 'render',
    controls: [
      { kind: 'slider', key: 'shadowEnd', label: t('暗部上界'), min: 0, max: 1, step: 0.005 },
      { kind: 'slider', key: 'specStart', label: t('高光下界'), min: 0, max: 1, step: 0.005 },
      {
        kind: 'slider',
        key: 'edgeSoftness',
        label: t('软边宽度'),
      ...
      { kind: 'slider', key: 'shadowMult', label: t('暗部亮度倍率'), min: 0, max: 1.5, step: 0.01 },
      {
        kind: 'slider',
        key: 'shadowMix',
        label: t('暗部染色比例'),
      ...
      { kind: 'slider', key: 'shadowSat', label: t('暗部饱和度'), min: 0, max: 2, step: 0.01 },
      { kind: 'slider', key: 'litSat', label: t('亮部饱和度'), min: 0, max: 2, step: 0.01 },
      { kind: 'slider', key: 'specMix', label: t('高光混色比例'), min: 0, max: 1, step: 0.01 },
      { kind: 'color', key: 'shadowTint', label: t('暗部染色（night-deep）') },
      { kind: 'color', key: 'specTint', label: t('高光混色（bone）') },
    ],
  },
  {
    id: 'outline',
    title: t('描边（inverted hull）'),
    open: true,
    side: 'right',
    tab: 'render',
    controls: [
      { kind: 'toggle', key: 'outlineEnabled', label: t('启用描边') },
      {
        kind: 'slider',
        key: 'outlineWidth',
        label: t('线宽（px @1080p）'),
      ...
      { kind: 'color', key: 'inkColor', label: t('描边色（禁止纯黑）') },
    ],
  },
  {
    id: 'halftone',
    ...
    tab: 'render',
    controls: [
      { kind: 'toggle', key: 'halftoneEnabled', label: t('启用网点') },
      { kind: 'slider', key: 'halftoneSize', label: t('网点尺寸（px）'), min: 2, max: 16, step: 0.5 },
      {
        kind: 'slider',
  ...
  },
  {
    ...
    tab: 'render',
    controls: [
      { kind: 'select', key: 'tonemapMode', label: t('Tonemap'), options: TONEMAP_OPTIONS },
      { kind: 'slider', key: 'exposure', label: t('曝光'), min: 0.1, max: 4, step: 0.01 },
      { kind: 'toggle', key: 'gradeEnabled', label: t('启用 Grading') },
      { kind: 'toggle', key: 'bloomEnabled', label: t('启用 Bloom') },
      { kind: 'slider', key: 'bloomThreshold', label: t('Bloom 阈值'), min: 0, max: 4, step: 0.01 },
      { kind: 'slider', key: 'bloomIntensity', label: t('Bloom 强度'), min: 0, max: 2, step: 0.01 },
      { kind: 'slider', key: 'vignette', label: t('暗角'), min: 0, max: 1, step: 0.01 },
    ],
  },
  {
    id: 'grading',
    title: t('Grading 三段调色'),
    open: false,
    side: 'right',
    tab: 'render',
    controls: [
      { kind: 'slider', key: 'gradeShadowRange', label: t('暗部上界'), min: 0, max: 1, step: 0.01 },
      { kind: 'slider', key: 'gradeMidRange', label: t('中间调上界'), min: 0, max: 1, step: 0.01 },
      { kind: 'slider', key: 'gradeEdge', label: t('段间过渡'), min: 0, max: 0.3, step: 0.005 },
      { kind: 'slider', key: 'gradeShadowMult', label: t('暗部倍率'), min: 0, max: 2, step: 0.01 },
      { kind: 'slider', key: 'gradeShadowMix', label: t('暗部染紫蓝'), min: 0, max: 1, step: 0.01 },
      { kind: 'slider', key: 'gradeShadowSat', label: t('暗部饱和度'), min: 0, max: 2, step: 0.01 },
      { kind: 'slider', key: 'gradeMidSat', label: t('中间调饱和度'), min: 0, max: 2, step: 0.01 },
      { kind: 'slider', key: 'gradeMidMult', label: t('中间调倍率'), min: 0, max: 2, step: 0.01 },
      { kind: 'slider', key: 'gradeLightMult', label: t('亮部倍率'), min: 0, max: 2, step: 0.01 },
      { kind: 'slider', key: 'gradeLightMix', label: t('亮部混 bone'), min: 0, max: 1, step: 0.01 },
      { kind: 'slider', key: 'gradeLightSat', label: t('亮部饱和度'), min: 0, max: 2, step: 0.01 },
    ],
  },
  {
    id: 'mesh-material',
    title: t('Mesh 材质（Material Slot）'),
    open: true,
    side: 'right',
    tab: 'inspector',
    controls: [],
  },
  {
    ...
    tab: 'render',
    controls: [
      { kind: 'select', key: 'editTarget', label: t('编辑目标'), options: EDIT_TARGETS },
      { kind: 'material-color', field: 'albedo', label: t('Albedo') },
      { kind: 'material-slider', field: 'roughness', label: t('粗糙度'), min: 0, max: 1, step: 0.01 },
      { kind: 'material-slider', field: 'metallic', label: t('金属度'), min: 0, max: 1, step: 0.01 },
      { kind: 'material-color', field: 'emissiveColor', label: t('自发光颜色') },
      {
        kind: 'material-slider',
  ...
  },
  {
    ...
    tab: 'render',
    controls: [
      { kind: 'select', key: 'debugMode', label: t('调试视图'), options: DEBUG_OPTIONS },
      {
        kind: 'slider',
        key: 'cameraElevation',
        label: t('相机俯仰'),
      ...
      { kind: 'toggle', key: 'autoOrbit', label: t('自动环绕') },
    ],
  },
];
