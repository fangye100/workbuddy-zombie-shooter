...
### 📖 MCP 使用文档（`tools/mcp-binding/README.md`）—— 2026-09-24 全面核对
新 Agent 上手看这两处：顶部 **「新 Agent 从这里开始」**（一句话流程 + 4 条坑）
与 **「完整实操流程」**（16 步表 + 3 个用户确认门）。
- 🔴 **画布尺寸不是固定的**：`.bd-stage .bd-canvas` 是 `flex:1 1 auto; width/height:100%`，
  随窗口变（见过 370×704 与 370×760）→ **旧常数 `185 + 305.4·u` / `699.2 − 305.4·y`
  只在某窗口尺寸下成立**。要标定就按 `computeViewFit()` 现算 + marker 圆点反验；
  **能用网格（铁律 0）就别标定**。
- 🔴 **12 方向包围会假 PASS**（测「周围有没有几何」，不是「点在不在肉里」）：
  Neck 改前（在空隙里）9/12、改后（在柱内）**也 9/12**。真判据 = **射线奇偶投票**。
- 🔴 **所有「在肉里」判据都判不了「在对的位置」** —— 判据只用于排除明显错误，
  定位靠解剖标志 + 骨段比 + 用户判读。
- **工具调用速查**节有 16 个工具的字面 JSON 示例（已实跑验证 16/16）+
  参数名清单（`set_joint{name,position}` / `mirror{dir}` / `cylinders{action,bone,seg,value,offset}` /
  `render{...grid,gridStep,gridMajor}` / `load_model{path}`）。
- 有 **3 个用户确认门**：门1 标杆侧关节 → 门2 镜像后骨架 → 门3 wrapper 圆柱。
- 镜像纪律：正确顺序 = **先将标杆侧调准 → 用户确认 → 再一次性镜像**；
  调整阶段不用 mirror 去「对齐」（那是拿一侧几何硬套另一侧）。

### 🏁 E-01 joint 阶段完成（2026-09-24，R2L 镜像后，commit `f17710e`）
**中轴 + 右半身为标杆，左侧 = 镜像（x 取反）**
...
- 10 对镜像对 Δ 全 = **0.000000**；右半身 0 变动；圆柱同步复制
- 🔴 **镜像代价**：3 个关节落在左侧网格外 2–4cm（网格左右不对称 2.4–4.9cm 的必然）——
  `LeftFoot` Δx 2.0cm、`LeftLeg` Δx1/Δz3cm、`LeftHand` Δz2.0cm。
  **均远小于对应圆柱半径（8–10cm）→ 蒙皮不受影响**，仅视觉上略偏出左侧轮廓。
- 🔴 **教训：用户说「镜像到另外一边」时方向要确认清楚。** 我先按 L→R 设计选项，
  还把用户专门定过的 `RightArm 1.650` 当成「要被覆盖的值」——方向反了，用户明确纠正为
  **R→L（右侧保留）**。用户此前对右侧的专门修正 = 右侧是标杆的信号。
...
### 🔁 胸腔 Spine2（2026-09-24，用户指「侧面胸腔靠后且太低」）
- 定稿（commit `7aee368`）：`Spine2 (0,1.450,-0.260)` → **`(0,1.600,-0.200)`**（用户指定）。
- 依据：中轴 `|x|<0.10` 的 z 连通块（背壳/前表面）——y=1.60 处背面 z=−0.315、前面 +0.114，
  用户的 −0.200 = **背面往前约 27% 深度** = 脊椎该在的位置（脊椎靠背，不在躯干中心）。
- **连带效应（意外收获）**：`Spine1→Spine2` 0.151→**0.310**、`Spine2→Neck` 0.438→**0.316**
  —— 从极不均衡的 0.15/0.44 变成接近的 0.31/0.32。用户凭视觉一次修好了两个问题。
- 验证：射线奇偶 6/6；z 合法性区间（y=1.600）=[−0.28, +0.04]，−0.20 居中偏后。
- ⚠️ 观察点：中轴间隔变成 0.11/0.15/**0.30**（Hips→Spine→Spine1→Spine2），
  Spine1→Spine2 段 0.310 明显最长，需留意蒙皮权重在胸段的跨度。

### 🔴 两个「在肉里」判据都**判不了「在对的位置」**
- 12 方向包围：测「周围有没有几何」→ 夹缝也高分，**会假 PASS**（Neck 改前改后都 9/12）
- 射线奇偶投票：测「点在不在封闭网格内」→ **真判据**，但
  Spine2 改前 (1.450,−0.260) 与改后 (1.600,−0.200) **都是 6/6**，**也区分不出高度**
- → 判据只用来**排除明显错误（悬空/穿出体外）**；**定位必须靠解剖标志**
  （真裆、颈柱、背深比例）+ 骨段比 + 用户判读。**判据不能用来「确认正确」。**
