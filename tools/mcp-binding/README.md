...
`render.ts` 的 auto-fit 把 **capsules 端点 ± 半径** 也纳入包围盒
（`extend(au, av, r)`）。自己写脚本出「网格 + 圆柱 + 关节十字 + 标签」叠加图时，
若变换只按 mesh 顶点算（E-01 得 scale 485.9），而渲染图含圆柱（实际 442.7），
**两层会错位十几厘米**（现象：`HD` 标签浮在头顶上方）。

修法 = 复刻完整 auto-fit：`points + segments端 + capsules端±max(r) + markers 0.02`。
自检：从渲染图里的关节 marker 圆点反解变换，与你的变换对表，rms 应 < 1px。

> 注：**关节阶段（`showCylinders:false`）的图不受此影响** —— 那时 capsules 为空。
> 这个坑只在 wrapper 阶段出叠加图时出现。
...
**⑤ 「12 方向射线包围」校验 —— 比投影 run 归属更强的判据**

在关节所在高度取**高模**横截层，从关节 (x,z) 向 12 个等分方向发射射线，
看是否都能在 0.32m 内打到几何（tol 2cm）：

```
E-01 定稿： Hips 12/12   LUpLeg 12/12   RUpLeg 11/12   LeftLeg 9/12   LFoot 12/12
```

`n/12` 近似「该关节被肉包围的立体角比例」。**12/12 或 11/12 = 在肉里；
≤9/12 = 边缘/贴皮；≤6/12 = 悬空。** 这个校验直接回答「关节在不在肉里」，
不依赖投影（投影只能证明「在剪影轮廓的某个 run 内」）。

⚠️ 注意：该校验**验证不了高度是否解剖正确**（改前的 0.900 也是 11–12/12，
因为它在大腿里面）。必须与 ①②③ 合起来用。
...
→ cylinders unpin（逐根）或直接手工给半径 → cylinders autoFit
   ...
   僵尸/破损模型上 autoFit 常**偏小到不可用**：E-01 的 Hips 骨长 0.186 → r=0.065，
   包不住 19.8cm 半宽的骨盆 → 改用密度峰值法，见上一小节）
→ setRadius/setOffset 迭代。**先查 offset 是否被乱拖**（E-01 清零 offset 就把
   未包裹从 51% 砍到 28%）；半径用密度峰值而非分位数
→ 算**权属分布**查跨部位抢权重（不只是看未包裹数）
→ 出正/侧全貌图（**投影必须含圆柱**，见上一小节）→ **发给用户核对，确认后才继续**
